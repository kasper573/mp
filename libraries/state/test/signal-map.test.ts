...
import { SignalMap } from "../src/signal-map";

describe("effects", () => {
  describe("can react to", () => {
    it("Map.clear", () => {
      testEffect({
        initialize: () => new SignalMap<string, string>([["1", "a"]]),
  ...
  it("can unsubscribe", () => {
    ...
    const map = new SignalMap<string, Entity>();
...
function testEffect<K, V>(opt: {
  initialize: () => SignalMap<K, V>;
  mutate: (map: SignalMap<K, V>) => void;
  getKey: K;
