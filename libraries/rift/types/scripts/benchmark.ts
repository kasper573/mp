...
boxplot(() => {
  summary(() => {
    group("signals: leaf", () => {
      bench("leaf set x1000 (same value)", () => {
        const sig = u32Ty.signal(0);
        sig.setRoot({ dirty: false });
        for (let i = 0; i < 1000; i++) {
          sig.set(0);
        }
        do_not_optimize(sig.peek());
      });
      bench("leaf set x1000 (changing)", () => {
        const sig = u32Ty.signal(0);
        sig.setRoot({ dirty: false });
        for (let i = 0; i < 1000; i++) {
          sig.set(i);
...
boxplot(() => {
  summary(() => {
    group("signals: composite", () => {
      bench("object signal create", () => {
        const sig = entityTy.signal(entityValue);
        sig.setRoot({ dirty: false });
        do_not_optimize(sig);
      });
      bench("object proxy mutate x1000", () => {
      ...
      bench("tuple indexed set x1000", () => {
        const sig = tupleTy.signal(tupleValue);
        sig.setRoot({ dirty: false });
        const p = sig.value as unknown as [number, string, number];
        for (let i = 0; i < 1000; i++) {
          p[0] = i & 0xff;
        }
        do_not_optimize(sig.peek());
      });
      bench("optional toggle x1000", () => {
      ...
      bench("union set x1000 (alternating tag)", () => {
        const sig = unionTy.signal({ tag: "move", value: { dx: 0, dy: 0 } });
        sig.setRoot({ dirty: false });
        for (let i = 0; i < 1000; i++) {
          if (i % 2 === 0) {
            sig.set({ tag: "move", value: { dx: i, dy: i } });
          } else {
            sig.set({ tag: "jump", value: { h: i } });
          }
        }
        do_not_optimize(sig.peek());
      });
      bench("transform set x1000", () => {
